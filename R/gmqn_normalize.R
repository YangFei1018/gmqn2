...
gmqn_normalize <- function(m, um, probe, type = '450k', ref = 'default', verbose = TRUE) {
  ...
  if (type == '450k') {
    ...
    t1.red <- row.names(annon_935k)[which(annon_935k$color == 'Red' & annon_935k$probe_type == 'I')]
    t1.green <- row.names(annon_935k)[which(annon_935k$color == 'Grn' & annon_935k$probe_type == 'I')]
    t2 <- row.names(annon_935k)[which(annon_935k$probe_type == 'II')]
    type <- annon_935k[probe, 'probe_type']
  }
