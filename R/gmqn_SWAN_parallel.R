...
#'
#' Using GMQN to conduct between array normalization and using SWAN to conduct within array normalization. We slightly modified swan function in minfi package. When the signal value is less than 0, we make it equal to the non-zero minimum value not the background signal intensity.
...
#' @param type The type of methylation array. . Supported values are "450k" (default), "850k", or "935k".
...
gmqn_swan_parallel <- function(m, um, type = '450k', ref = 'default', ncpu = 4, verbose = TRUE) {
  ...
  if (type == '450k') {
    ...
    m.tem = matrix(nrow = 930075, ncol = ncol(m))
    m.tem = data.frame(m.tem)
    row.names(m.tem) = row.names(annon_935k)
  } else {
  ...
  beta.GMQN.swan = foreach (i=1:ncol(m), .combine=cbind) %dopar% {
    ...
    res = gmqn2::gmqn_swan(m[no.na.index,i], um[no.na.index,i], row.names(m)[no.na.index], type = type, ref = ref)
