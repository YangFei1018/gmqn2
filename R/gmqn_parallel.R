#' GMQN in parallel
#'
#' Using GMQN to conduct between array normalization.
...
gmqn_parallel <- function(m, um, type = '450k', ref = 'default', ncpu = 4, verbose = TRUE) {
  ...
  beta.GMQN = foreach (i=1:ncol(m), .combine=cbind) %dopar% {
    beta = rep(NA, nrow(m))
    no.na.index = which(is.na(m[,i]) == F & is.na(um[,i]) == F)
    res = gmqn2::gmqn_normalize(m[no.na.index,i], um[no.na.index,i], row.names(m)[no.na.index], type = type, ref = ref)
    beta.tmp <- res$m / (res$m + res$um)
    beta.tmp[which(res$p >= 0.01)] = NA
    beta[no.na.index] = beta.tmp
    beta
  }
  beta.GMQN = data.frame(beta.GMQN)
  names(beta.GMQN) = names(m)
  row.names(beta.GMQN) = row.names(m)
  return(beta.GMQN)
}
